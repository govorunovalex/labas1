...
public:
  ...
  TSet Reminders (int K) const;           // Доп. Задание 1, оставляем только делящиеся на К
  bool WriteToFile (const std::string& filename) const; // Доп. Здадание 2, запись и чтение из файла
  bool ReadFromFile (const std::string& filename); // Доп. Здадание 2, запись и чтение из файла
  void Update(const int index, const int NewElem); // Доп. Задание 3, изменить элемент по месту
